...
import { LayoutGrid, TrendingUp, TrendingDown, Compass } from 'lucide-react';
    ...
    >
      ...
      <div className="flex items-center justify-between border-b pb-3 mb-3.5 border-inherit">
        <div className="flex items-center gap-2 min-w-0">
          <div className={`p-2 rounded-xl shrink-0 ${isDark ? 'bg-cyan-950/80 text-cyan-400 border border-cyan-800/50' : 'bg-purple-100 text-purple-700 border border-pink-200'}`}>
          ...
          <div className="min-w-0">
            <h3 className={`text-xs sm:text-sm font-black uppercase tracking-tight truncate ${isDark ? 'text-slate-100' : 'text-purple-950'}`}>
              🔍 Çoklu Sembol Taraması & Radar
            </h3>
            <p className={`text-[10px] sm:text-[11px] font-medium truncate ${isDark ? 'text-slate-400' : 'text-purple-700'}`}>
              Popüler Paritelerin Tako Skorları & Sinyal Güçleri
            </p>
          </div>
        </div>

        <span className={`text-[10px] sm:text-xs font-black px-2 py-0.5 rounded-full border shrink-0 ${isDark ? 'bg-slate-800 text-cyan-400 border-slate-700' : 'bg-pink-100 text-pink-700 border-pink-200'}`}>
          {screenerItems.length} Parite
        </span>
      </div>
        ...
        {screenerItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.divergenceTag && (
                <div className="mb-1.5 flex items-center gap-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-300 w-fit">
              ...
              <div className="flex items-center justify-between mb-1.5 gap-2">
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className={`font-black text-xs sm:text-sm truncate ${isDark ? 'text-slate-100' : 'text-purple-950'}`}>
                    {item.symbol.base}
                  </span>
                  <span className={`text-[9px] sm:text-[10px] px-1 py-0.2 rounded font-bold shrink-0 ${
                    item.change24h >= 0
                ...
                <span className={`font-mono text-xs font-black shrink-0 ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>
                  ${item.price.toLocaleString(undefined, { minimumFractionDigits: item.symbol.decimals })}
                </span>
              </div>

              {/* Signal Badge & Score */}
              <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-inherit">
                <div className="flex items-center gap-1">
                  <span className={`px-1.5 py-0.5 rounded-lg text-[10px] sm:text-xs font-black border flex items-center gap-1 ${
                    isLong
                    ...
                    {isLong && <TrendingUp className="w-3 h-3 text-emerald-600" />}
                    {isShort && <TrendingDown className="w-3 h-3 text-rose-600" />}
              ...
              <div className="w-full mt-1.5 bg-pink-100/50 h-1 rounded-full overflow-hidden border border-pink-200/60 p-0.5">
                <div
