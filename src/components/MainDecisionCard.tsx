...
export const MainDecisionCard: React.FC<MainDecisionCardProps> = ({
  ...
  return (
    <div className="relative overflow-hidden bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-2xl backdrop-blur-xl">
      ...
      {/* Top Header Row: Symbol & Current Live Price & v3.2 Pills */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4 mb-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* v3.2 Hyperliquid DEX Price Pill */}
          {signal.hlPrice && (
            <div className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-cyan-950/80 text-cyan-300 border border-cyan-500/30 font-mono flex items-center gap-1.5">
              <span>Hyperliquid DEX:</span>
              <span className="text-slate-100">${signal.hlPrice}</span>
              <span className="opacity-80">({signal.hlDivergencePct && signal.hlDivergencePct >= 0 ? '+' : ''}{signal.hlDivergencePct}%)</span>
            </div>
          )}

          {/* v3.2 24h Whale Netflow Pill */}
          {signal.netflowUsd !== undefined && (
            <div className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-indigo-950/80 text-indigo-300 border border-indigo-500/30 font-mono">
              Cüzdan Netflow: {signal.netflowUsd > 0 ? '+' : ''}${(signal.netflowUsd / 1e6).toFixed(1)}M
            </div>
          )}
        </div>

        <div className="text-right">
          <div className="text-xs text-slate-400 font-medium uppercase tracking-wider flex items-center justify-end gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-5 flex flex-col items-center justify-center p-6 bg-slate-950/80 rounded-2xl border border-slate-800/80 shadow-inner text-center relative">
          ...
          {/* Confidence & Risk Level & v3.2 Kelly & Signal Strength Row */}
          <div className="w-full grid grid-cols-2 gap-2 mt-4 text-xs font-semibold">
            ...
            <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-left">
              <span className="text-slate-400 block text-[10px] uppercase">Kelly Risk Önerisi</span>
              <span className="text-cyan-400 font-extrabold text-sm">%{signal.kellyFraction ?? 5.0} Sermaye</span>
            </div>
          </div>

          {/* v3.2 Sinyal Gücü Endeksi Bar */}
          <div className="w-full mt-3 p-3 bg-slate-900/90 rounded-xl border border-slate-800/90 text-left space-y-1.5">
            <div className="flex justify-between items-center text-[10px] uppercase font-bold text-slate-400">
              <span className="flex items-center gap-1">
                <Zap className="w-3 h-3 text-amber-400" />
                Sinyal Gücü Endeksi (SSI)
              </span>
              <span className="text-amber-400 font-extrabold">{signal.signalStrengthIndex ?? 72}/100</span>
            </div>
            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800 p-0.5">
              <div
                ...
                style={{ width: `${signal.signalStrengthIndex ?? 72}%` }}
              />
            </div>
            <div className="flex justify-between text-[9px] text-slate-500 font-mono">
              <span>Brier: {signal.brierScore ?? 0.160}</span>
              <span>MAD Robust Z-Score Verified</span>
            </div>
          </div>
        </div>
